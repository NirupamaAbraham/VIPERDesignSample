        ...
        request.delegate.queue.addOperation {
            if let response = request.response, request.error == nil {
                if !codesObject.contains(response.statusCode) {
                    request.delegate.error = NSError(domain: AKNetworkingErrorDomain, code: response.statusCode, userInfo: nil)
                }
    ...
    private func validateHeaders(_ headersObject: [String : String], request: Request) {
        request.delegate.queue.addOperation {
            if let response = request.response, request.error == nil {
                ...
                if !result {
                    request.delegate.error = NSError(domain: AKNetworkingErrorDomain, code: 415, userInfo: nil)
                }
        ...
        request.delegate.queue.addOperation {
            if let data = request.data, request.error == nil {
                if let responseString = NSString(data: data, encoding: String.Encoding.utf8.rawValue) ,
                    ...
                    request.delegate.error = NSError(domain: AKNetworkingErrorDomain, code: 420, userInfo: nil)
                }
