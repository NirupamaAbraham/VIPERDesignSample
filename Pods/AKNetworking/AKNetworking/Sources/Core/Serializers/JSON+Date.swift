...
extension JSON {
    ...
    public func dateNormalizedToGMTFromFormat(_ format: String) -> Date? {
        ...
        case .string:
            ...
            let formatLength = format.replacingOccurrences(of: "'", with: "", options: [], range: nil).characters.count
            if length >= formatLength {
                let endIndex = string.characters.index(string.startIndex, offsetBy: formatLength)
